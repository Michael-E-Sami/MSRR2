...
public:
  ...
  void magnetism(std_msgs::String faceToAttach)
  {
    ...
    std::string firstname = faceToAttach.data.substr(0, pos),
      ...
      ///
    int pos1 = firstname.find_first_of('/'); //first ditto
    std::string firstname1 = firstname.substr(0, pos1),
      lastname1 = firstname.substr(pos1+1); //body
    ...
    if (this->model->GetName()== firstname1)
    {
      ...
      { ROS_INFO("failed here 1");
        ...
        {
          if((lastname1 == "body")&&(isattached_body)){
            return;
          }
          if((lastname1 == "front_face")&&(isattached_front_face)){
            return;
          }
          if((lastname1 == "left_wheel")&&(isattached_left_wheel)){
            return;
          }
          if((lastname1 == "right_wheel")&&(isattached_right_wheel)){
            return;
          }
          ...
          if(lastname1 == "front_face"){
            isattached_front_face = true;
          }
          if(lastname1 == "left_wheel"){
            isattached_left_wheel = true;
          }
          if(lastname1 == "right_wheel"){
            isattached_right_wheel = true;
          }

          ROS_INFO("failed here 3");
          this->fixedJoint = this->model->CreateJoint("fixedjoint" + lastname1, "revolute", this->model->GetLink(lastname1), this->someLink);
