...
def rotlecall(data,name):
    # rospy.loginfo(rospy.get_caller_id() + "I heard ")
    #global current_x
    #global current_y
    #global current_yaw
	
	robotDict["ditto" + str(name) + "/current_x"] = data.pose.pose.position.x
	robotDict["ditto" + str(name) + "/current_y"] = data.pose.pose.position.y
	#robotDict["ditto" + str(name) + "/rot_q"] = data.pose.pose.orientation
    #current_y = data.pose.pose.position.y
    #current_x = data.pose.pose.position.x
    rot_q = data.pose.pose.orientation
    (roll, pitch, robotDict["ditto" + str(name) + "/current_yaw"]) = euler_from_quaternion(
        rot_q.x, rot_q.y, rot_q.z, rot_q.w])

    robotDict["ditto" + str(name) + "/current_yaw"] = robotDict["ditto" + str(name) + "/current_yaw"]*(180/pi)

    if robotDict["ditto" + str(name) + "/current_yaw"] < 0:
        robotDict["ditto" + str(name) + "/current_yaw"] += 360
##########################################################################
def callbackFrw(data,name):
    #global frw_x
    #global frw_y
    #global frw_yaw
	
	robotDict["ditto" + str(name) + "/frw_x"] = data.pose.pose.position.x
	robotDict["ditto" + str(name) + "/frw_y"] = data.pose.pose.position.y
	rotFrw = data.pose.pose.orientation
	(rotFrwroll, rotFrwpitch, robotDict["ditto" + str(name) + "/frw_yaw"]) = euler_from_quaternion(
        [rotFrw.x, rotFrw.y, rotFrw.z, rotFrw.w])
	#robotDict["ditto" + str(name) + "/rotFrw"] = data.pose.pose.orientation
...
def rotate(omega, angle, ditto):
    ...
    rospy.Subscriber("/ditto" + ditstr + "/odom", Odometry, rotlecall, ditto)
    ...
    while start:
        print('yaw goal - current yaw := ', yaw_goal - robotDict["ditto" + ditstr + "/current_yaw"]) 
        if abs(yaw_goal - robotDict["ditto" + ditstr + "/current_yaw"]) > 0.1:
            ...
            if (angle > 0):
                ...
                omega = 0.025
                if yaw_goal - robotDict["ditto" + ditstr + "/current_yaw"] > 0 :
                    print '4 := ', yaw_goal
                    left_pub.publish(-omega)
                    right_pub.publish(omega)
                else:
...
def forwardFn(Distance, Omega, dittoNum):
    ...
    rospy.Subscriber("ditto" + frwDitto + "/odom", Odometry, callbackFrw, dittoNum)
    ...
    goal = Point()
    goal.x= distance *cos(robotDict["ditto" + str(name) + "/frw_yaw"]) + robotDict["ditto" + str(name) + "/frw_x"]
    goal.y= distance *sin(robotDict["ditto" + str(name) + "/frw_yaw"])  + robotDict["ditto" + str(name) + "/frw_y"]

    while start2:
        FrwDiff_x = goal.x - robotDict["ditto" + str(name) + "/frw_x"]
        FrwDiff_y = goal.y - robotDict["ditto" + str(name) + "/frw_y"]
