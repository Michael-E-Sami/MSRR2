...
def callback0(msg,args):
        ...
        if first_pol == second_pol:
            ...
            if first[0]== "ditto"+ movingDitto[0] and second[0]== "ditto"+ stableDitto[0]:
                isattached =True
                print'line 54'
            elif first[0]== "ditto"+ stableDitto[0] and second[0]== "ditto"+ movingDitto[0]:
                isattached =True
                print'line 57'
        else:
            tree= first[0] + "/"+first_body +"#" + second[0]+"/"+second_body
...
def rotate(angle, Omega, dittoNum):
    ...
    while start:
        ...
        print 'abs yaw goal - current yaw = ', abs(yaw_goal - currentYawDeg[indexRot])
        #rospy.sleep(10)
        # print('yaw goal - current yaw = ', yaw_goal - currentYawDeg[indexRot])
        # print 'abs yaw goal - current yaw = ', abs(yaw_goal - currentYawDeg[indexRot])
        # rospy.sleep(10)
        ...
        # print 'abs yaw goal - current yaw = ', abs(yaw_goal - currentYawDeg[indexRot])
        # rospy.sleep(10)
        if abs(yaw_goal - currentYawDeg[indexRot]) > 0.5:
...
def main():
    ...
    rospy.Subscriber("/ditto3/sensor_data",String,callback0,"ditto3")
    ##############
    ...
    tempX1 = (0.2*cos(lastAngleRad)) + currentX[1]
    tempY1 = (0.2*sin(lastAngleRad)) + currentY[1]

    #forwardFn(tempX1, tempY1, 0.15, str(stableDitto))\
    forwardFnDist(0.18,0.15,str(stableDitto))
